...
// Reference to kproxui object
var kproxui;

// Reference to KProx class
var KProx;
...
/* exported userMark */
var userMark = (function () {

    /* global is: true, dom: true */
    ...
    document.addEventListener('DOMContentLoaded', function () {
        dom.append(document.body, '<div id="userMarks"></div>');
        overlay = document.getElementById('userMarks');
        domReady = true;
    });
    window.addEventListener('resize', function () {
        for (var selector in marks) {
            if (marks.hasOwnProperty(selector)) {
                var boxElement = dom.offset(marks[selector].element);
                marks[selector].mark.style.top    = parseInt(boxElement.top - 12, 10) + 'px';
                marks[selector].mark.style.left   = parseInt(boxElement.left - 2, 10) + 'px';
                marks[selector].mark.style.width  = parseInt(boxElement.width + 3, 10) + 'px';
                marks[selector].mark.style.height = parseInt(boxElement.height + 13, 10) + 'px';
            }
        }
    });
    ...
    return {
        ...
        add: function (selector, name) {
            ...
            var boxElement = dom.offset(element);
            //TODO: En IE9 e IE10 no funciona correctamente, ya que no el estilo pointer-events: none no es aceptado
//            dom.append(overlay, '<div id="mark' + (++num) + '" ' +
//                                'style="pointer-events: none; position: absolute; border: 1px dotted red; ' +
//                                'top: ' + parseInt(boxElement.top - 12, 10) + 'px; ' +
//                                'left: ' + parseInt(boxElement.left - 2, 10) + 'px; ' +
//                                'height: ' + parseInt(boxElement.height + 13, 10) + 'px; ' +
//                                'width: ' + parseInt(boxElement.width + 3, 10) + 'px;"></div>');
            var top = parseInt(boxElement.top, 10) - 2;
            var left = parseInt(boxElement.left, 10) - 2;
            var height = parseInt(boxElement.height, 10) + 4;
            ...
            marks[selector] = {
                element: element,
                mark: document.getElementById('mark' + num),
                top: document.querySelector('#mark' + num + ' .top' ),
                left: document.querySelector('#mark' + num + ' .top' ),
...
kproxui = (function () {// Reference to model create with kprox.newDocument()
    var initialData = {};   // Initial data getting for the HTML

    /**
     * Get the kproxui name for one element
     * by priority the kproxui name for one element is kprox-model, id or name attribute
     * except radio button than uses kprox-model and name
     * @param {HTMLElement} element
     * @returns {string} name of this element
     */
    function getName(element) {
        if (element.tagName === 'INPUT' && element.getAttribute('type') === 'radio') {
            return element.getAttribute('kprox-model') || element.getAttribute('name');
        }
        return element.getAttribute('kprox-model') || element.getAttribute('id');
    }
    // End of getName

    function getElement(id) {
        var result;
        result = document.querySelector('[kprox-model=' + id + ']');
        if (result) {
            return result;
        }
    ...
    /**
     * Get info and add event handle to a form elements
     * @param {HTMLElement} element
     */
    function formElement(element) {
        if (element.tagName === 'SELECT' || element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
            var id = getName(element);
            if (id === null || element.getAttribute('kprox-config') === 'ignore') {
                return;
            }
            obj.setDepth(initialData, id, dom.val(element));
            dom.onChange(element, function (e) {
               model.call(this, function (model) {
                    obj.setDepth(model, id, dom.val(this));
                }, {userEvent: true});
            });
            element.addEventListener('focus', function () {
                model.lock(id);
                userMark.add(this, 'me');
            });
            element.addEventListener('blur', function () {
                model.unlock(id);
                userMark.remove(this);
            });
        }
    }
    // End of formElement
    ...
    /**
     * Get info and add event handler to a html element
     * @param {HTMLElement} element
     */
    function domElement(element) {
        ...
        obj.setDepth(initialData, id, {});
        for (var a = 0; a < element.attributes.length; a++) {
            var attribute = element.attributes[a];
            if (attribute.nodeName !== 'id' && attribute.nodeName !== 'kprox-model' && attribute.nodeName !== 'name') {
                obj.setDepth(initialData, id + '.' + attribute.nodeName, attribute.value);
            }
        }
        dom.onAttrModified(element, function (attr) {
            model.call(this, function(model) {
                obj.setDepth(model, getName(this) + '.' + attr, this.getAttribute(attr));
            }, {userEvent: true});
        });
    }
    // End of domElement
